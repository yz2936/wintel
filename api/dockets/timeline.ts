...
export default async function handler(req: IncomingMessage & { body?: any }, res: ServerResponse) {
  try {
    const { generateDocketTimeline, requireUser } = await import('./_shared.js');
    ...
    const result = await generateDocketTimeline(
      user.id,
      body?.state === 'MA' ? 'MA' : 'NY',
    ...
    const message = error instanceof Error ? error.message : 'Unhandled timeline generation failure';
