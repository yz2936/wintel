...
export async function requireUser(headers?: Record<string, string | string[] | undefined>) {
  ...
  const { supabaseUrl, apiKey } = getSupabaseServerConfig();
...
export async function fetchPlanOfAttack(reportText: string, userProfile: string) {
  const prompt = `
...
export async function fetchTimeline(opcos: string[], functions: string[], year?: number | null) {
  const targetContext = opcos.length > 0
    ? `the following specific Operating Companies (OpCos): ${opcos.join(', ')}`
    : 'the broader US utility and energy industry';
...
async function requestText({ input }: { input: string }) {
...
function getSupabaseServerConfig() {
  ...
  if (!supabaseUrl || !apiKey) {
    throw new Error('Missing Supabase server environment variables for AI route auth.');
  }
